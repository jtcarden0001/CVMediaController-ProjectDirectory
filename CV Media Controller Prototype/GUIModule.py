...
class VideoFrame(tk.Frame):
    ...
    def __init__(self, root):
        ...
        self.start_button.grid(row=1, column=9, columnspan=1, padx=8)
        self.mode_label = tk.Label(self, text="Mode: ")
        self.mode_label.grid(row=2, column=0, columnspan=1, padx=8)
        self.face_select = tk.Radiobutton(self, text="Facial Recognition", variable=self.mode, value=1)
        self.face_select.grid(row=2, column=1, columnspan=1, padx=8)
        self.gesture_select = tk.Radiobutton(self, text="Gesture Recognition", variable=self.mode, value=2)
        self.gesture_select.grid(row=3, column=1, columnspan=1, padx=8)
        self.status_label = tk.Label(self, textvariable=self.status_message)
        self.status_label.grid(row=4, column=0, columnspan=1, padx=8)
